    ...
    try {
      if (tab === "all") {
        const res = await axios.get(
          "https://resqreliefcheck.onrender.com/api/requests/ai-prioritized",
        );
        setRequests(res.data);
      } else if (tab === "fraud") {
        const res = await axios.get(
          "https://resqreliefcheck.onrender.com/api/banned",
        );
        setBanned(res.data);
        setRequests([]);
      } else {
        ...
        const res = await axios.get(
          `https://resqreliefcheck.onrender.com/api/requests/by-status/${tab}`,
        );
        setRequests(res.data);
      }
    } catch {
                ...
                <tbody>
                  {requests.map((req, index) => (
                    <tr
                      ...
                      onClick={() =>
                        navigate(`/admin-requests/${req._id}`, {
                          state: { req },
                        })
                      }
                    >
            ...
            <table className="inv-table">
              <thead>
                <tr>
                  ...
                  <th>Status</th>
                  {(activeTab === "verified" ||
                    activeTab === "in_progress") && <th>Assigned To</th>}
                  <th>Submitted</th>
                </tr>
              </thead>
              <tbody>
