...
const VolunteerAssignment = () => {
  ...
  const handleStartWork = async () => {
    ...
    try {
      ...
      setFlash({
        type: "success",
        msg: "You are now marked as working on this request. Admin has been notified.",
      });
    } catch (err) {
      const msg =
  ...
  const handleMarkDone = async () => {
    if (
      !window.confirm(
        "Mark this request as done? Admin will be notified to review and close it.",
      )
    )
      return;
    setMarking(true);
    try {
      ...
      setFlash({
        type: "success",
        msg: " Great work! Admin has been notified to close this request.",
      });
    } catch (err) {
      const msg =
        err.response?.data?.message ||
    ...
    <div style={s.page}>
      <div style={s.wrap}>
        ...
        {!isAssigned && status !== "completed" && (
          <div style={s.alert}>
            ...
            <small style={{ opacity: 0.7 }}>
              Assigned: {req.assignedVolunteer?.email || "None"} · Your email:{" "}
              {volunteerEmail || "Not detected"}
            </small>
          </div>
        )}
        ...
        <div style={{ marginBottom: "24px" }}>
          <h1
            style={{ margin: "0 0 8px 0", fontSize: "24px", color: "#1a1a1a" }}
          >
            {req.district}
          ...
          <div style={s.card}>
            ...
            {status === "verified" && (
              <div>
                ...
                >
                  You have been assigned to this request. Tap below when you
                  begin working on it — admin will be notified.
                </p>
                <button
            ...
            {status === "in_progress" && (
              <div>
                ...
                >
                  You are currently working on this request. Once you have
                  completed all tasks, mark it as done below.
                </p>
                <button
            ...
            >
              {!req.inquiries || req.inquiries.length === 0 ? (
                <p style={{ color: "#aaa", fontSize: "14px" }}>
                  No messages yet. Use this to ask admin anything about this
                  request.
                </p>
              ) : (
              ...
              <div style={{ textAlign: "left", marginTop: "24px" }}>
                ...
                {req.inquiries.map((msg, i) => (
                  <div key={i} style={s.bubble(msg.from)}>
                    ...
                    <span style={s.bubbleMeta}>
                      {msg.from === "volunteer"
                        ? req.assignedVolunteer?.name || "Volunteer"
                        : "Admin"}{" "}
                      · {timeAgo(msg.sentAt)}
                    </span>
                  </div>
