...
const fetchFraudAnalysis = async () => {
  ...
  useEffect(() => {
    if (!req) fetchReq(); else fetchReq();
    fetchRequestTasks();
    fetchFraudAnalysis();

    socketRef.current = io(BASE, { transports: ["websocket", "polling"] });
    socketRef.current.on("connect", () => socketRef.current.emit("join", adminEmail));
    socketRef.current.on("alert", () => { fetchReq(); fetchRequestTasks(); });
...
const handleVerify = async () => {
  ...
  setAssignError("");
  try {
    const res = await axios.put(`${BASE}/api/requests/${id}/verify`, { assignedVolunteer: null });
    setReq(res.data);
  } catch (err) {
  ...
  const handleSendReply = async () => {
    ...
    try {
      const res = await axios.post(`${BASE}/api/requests/${id}/inquiry/reply`, {
        message: replyText.trim(), adminEmail, adminName,
      });
      setReq(res.data);
      setReplyText("");
  ...
  const handleAiResourceAllocation = async () => {
    ...
    try {
      const res = await axios.post(`${BASE}/api/ai/resource-allocation`, {
        aidTypes: req.aidTypes, peopleAffected: req.peopleAffected,
        district: req.district, description: req.additionalDetails,
      });
...
const handleAiVolunteerMatch = async () => {
  ...
  try {
    ...
    if (taskVolunteers.length === 0)
      axios.get(`${BASE}/api/volunteers/all`)
        .then((r) => setTaskVolunteers(r.data.filter((v) => v.volunteerRole)));
  ...
  const handleAiDonationAllocation = async () => {
    ...
    try {
      const res = await axios.post(`${BASE}/api/ai/donation-allocation`, {
        district: req.district, aidTypes: req.aidTypes,
        peopleAffected: req.peopleAffected, priority: req.priority,
        description: req.additionalDetails,
      });
      setAiDonation(res.data);
    } catch { alert("AI donation allocation failed. Try again."); }
    finally { setLoadingAiDonation(false); }
  };
  ...
  const statusColor = (s) =>
    s === "pending" ? "#c0392b" : s === "verified" ? "#1d4ed8" :
    s === "in_progress" ? "#b45309" : s === "volunteer_done" ? "#16a34a" :
  ...
  if (!req) return <div style={{ padding: "40px", color: "#888", textAlign: "center", fontSize: "18px" }}>Loading Request Details...</div>;
    ...
    return (
      <div className="ard-done-screen">
        <div className="ard-done-card">
          ...
          <button className="btn-admin" onClick={() => navigate("/admin-requests")}>← Back to Dashboard</button>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="detail-page">
        <div className="detail-header">
          <button className="btn-admin" style={{ padding: "10px 18px", background: "#fff", color: "#1a1a2e", border: "1.5px solid #ddd" }} onClick={() => navigate("/admin-requests")}>
            ← Back
          </button>
        ...
        <div style={{ background: "#fff", border: "1px solid #e8e4dc", borderRadius: "12px", padding: "40px", textAlign: "center", maxWidth: "500px", margin: "0 auto" }}>
          ...
          <button className="btn-admin" style={{ marginTop: "20px" }} onClick={() => navigate("/admin-requests")}>← Back to Requests</button>
        </div>
      </div>
    );
  }
  ...
  const assignedVolunteersFromTasks = (() => {
    ...
    return requestTasks
      ...
      .filter((t) => { if (seen.has(t.assignedTo.volunteerEmail)) return false; seen.add(t.assignedTo.volunteerEmail); return true; })
      .map((t) => ({ name: t.assignedTo.volunteerName, email: t.assignedTo.volunteerEmail }));
  })();
  ...
  const canAssignTask = ["pending", "verified", "in_progress"].includes(req.status);
  ...
  return (
    <div className="detail-page">
          ...
          <div className="ai-modal-container" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="ai-modal-body">
              ...
              {activeAiView === "resource" && (
                <div>
                  {loadingAiRes ? <p style={{ color: "#888", fontSize: "15px" }}>Analyzing required resources...</p> : 
                    aiResources ? (
                      <div>
                        <p style={{ color: "#555", fontStyle: "italic", marginBottom: "20px", fontSize: "16px", lineHeight: "1.6" }}>{aiResources.summary}</p>
                        <table className="detail-table" style={{ fontSize: "15px" }}>
                          <thead><tr><th style={{ color: "#888" }}>Item</th><th style={{ color: "#888" }}>Quantity Required</th><th style={{ color: "#888" }}>Reasoning</th></tr></thead>
                          <tbody>{aiResources.recommendations.map((r, i) => (<tr key={i}><td><strong>{r.item}</strong></td><td style={{ whiteSpace: "nowrap" }}>{r.quantity}</td><td style={{ color: "#555" }}>{r.reason}</td></tr>))}</tbody>
                        </table>
                      </div>
                    ) : (
                      <button className="btn-admin" style={{ padding: "12px 24px", fontSize: "15px" }} onClick={handleAiResourceAllocation}>Generate Recommendation</button>
                    )
                  }
                </div>
              )}
              ...
              {activeAiView === "volunteer" && (
                <div>
                  {loadingAiVol ? <p style={{ color: "#888", fontSize: "15px" }}>Scanning database...</p> : 
                    aiVolMatch ? (
                      <div>
                        <p style={{ color: "#555", fontStyle: "italic", marginBottom: "20px", fontSize: "16px", lineHeight: "1.6" }}>{aiVolMatch.summary}</p>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                        
                          {aiVolMatch.matches.map((match, i) => (
                            <div key={i} style={{ background: "#fff", borderRadius: "10px", padding: "20px", border: "1px solid #ddd8ce" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                                <p style={{ fontWeight: "700", margin: 0, fontSize: "18px" }}>{match.name}</p>
                                <div style={{ background: match.matchScore >= 85 ? "#16a34a" : match.matchScore >= 60 ? "#d97706" : "#888", color: "#fff", borderRadius: "999px", padding: "6px 14px", fontWeight: "700", fontSize: "14px" }}>
                                  {match.matchScore}% Match
                                </div>
                              </div>
                              <p style={{ color: "#666", fontSize: "15px", lineHeight: "1.5" }}>{match.reason}</p>
                              <button
                                onClick={() => handleAiAssignVolunteer(match)}
                                style={{ marginTop: "12px", width: "100%", padding: "8px", background: "#c0392b", color: "#fff", border: "none", borderRadius: "8px", fontWeight: 600, fontSize: "14px", cursor: "pointer" }}
                              >
                      ...
                      <button className="btn-admin" style={{ padding: "12px 24px", fontSize: "15px" }} onClick={handleAiVolunteerMatch}>Find Best Matches</button>
                    )
                  }
                </div>
              )}
              ...
              {activeAiView === "donation" && (
                <div>
                   {loadingAiDonation ? <p style={{ color: "#888", fontSize: "15px" }}>Calculating budget...</p> : 
                     aiDonation ? (
                      <div>
                        <div style={{ display: "flex", gap: "20px", marginBottom: "24px" }}>
                          <div style={{ flex: 1, background: "#f0fdf4", border: "1px solid #86efac", borderRadius: "10px", padding: "20px" }}>
                            <p style={{ fontSize: "14px", color: "#16a34a", margin: 0, fontWeight: 700, marginBottom: "8px" }}>AVAILABLE FUNDS</p>
                            <p style={{ fontSize: "28px", fontWeight: 700, margin: 0, color: "#166534" }}>৳{aiDonation.availableFunds?.toLocaleString()}</p>
                          </div>
                          <div style={{ flex: 1, background: "#fef3c7", border: "1px solid #fde68a", borderRadius: "10px", padding: "20px" }}>
                            <p style={{ fontSize: "14px", color: "#b45309", margin: 0, fontWeight: 700, marginBottom: "8px" }}>ESTIMATED NEEDED</p>
                            <p style={{ fontSize: "28px", fontWeight: 700, margin: 0, color: "#92400e" }}>৳{aiDonation.estimatedTotal?.toLocaleString()}</p>
                          </div>
                        </div>

                        <table className="detail-table" style={{ fontSize: "15px", marginBottom: "20px" }}>
                          <thead>
                            <tr>
                              <th style={{ color: "#888", padding: "12px" }}>Category</th>
                              <th style={{ color: "#888", padding: "12px" }}>Amount</th>
                              <th style={{ color: "#888", padding: "12px" }}>Reasoning</th>
                            </tr>
                          </thead>
                          <tbody>
                            {aiDonation.breakdown?.map((b, i) => (
                              <tr key={i}>
                                <td style={{ padding: "12px" }}><strong style={{ textTransform: "capitalize" }}>{b.category}</strong></td>
                                <td style={{ padding: "12px", whiteSpace: "nowrap" }}>৳{b.amount?.toLocaleString()}</td>
                                <td style={{ padding: "12px", color: "#555" }}>{b.reason}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>

                        <p style={{ fontStyle: "italic", color: "#666", fontSize: "15px", lineHeight: "1.7" }}>
                          {aiDonation.reasoning}
                        </p>
                      </div>
                     ) : (
                       <button className="btn-admin" style={{ padding: "12px 24px", fontSize: "15px" }} onClick={handleAiDonationAllocation}>Suggest Allocation</button>
                     )
                   }
                </div>
              )}
        ...
        <div className="ard-overlay">
          <div className="ard-modal">
            {confirmModal.type === "verify" ? (
              ...
              <>
                <h3>Mark as Fraud?</h3>
                <p>This will <strong>permanently delete</strong> the request and <strong>ban</strong> the phone number <code>{req.phoneNumber}</code>.</p>
                <div className="ard-modal-btns">
          ...
          <div className="ard-modal" style={{ maxWidth: "620px", width: "95%" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
              <div>
                ...
                <input value={taskForm.title} onChange={(e) => setTaskForm({ ...taskForm, title: e.target.value })} placeholder="e.g. Distribute food packs" style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box" }} />
              </div>
              <div>
                ...
                <select value={taskForm.taskType} onChange={(e) => setTaskForm({ ...taskForm, taskType: e.target.value })} style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box" }}>
              ...
              <div style={{ gridColumn: "1 / -1" }}>
                ...
                <textarea value={taskForm.description} onChange={(e) => setTaskForm({ ...taskForm, description: e.target.value })} placeholder="Describe the task in detail..." rows={3} style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", resize: "vertical", fontFamily: "inherit", boxSizing: "border-box" }} />
              </div>
              <div>
                ...
                <select value={taskForm.assignedTo.volunteerEmail} onChange={handleTaskVolunteerSelect} style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box" }}>
                  <option value="">— Unassigned —</option>
                  {taskVolunteers.map((v) => (
                    <option key={v._id} value={v.email}>{v.fullName} · {v.volunteerRole} · {v.preferredZone}{v.status === "confirmed" ? " ✓" : ""}</option>
                  ))}
                </select>
              </div>
              <div>
                ...
                <input value={taskForm.zone} onChange={(e) => setTaskForm({ ...taskForm, zone: e.target.value })} placeholder="e.g. Dhaka, Sylhet North" style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box" }} />
              </div>
              <div>
                <label style={{ fontSize: "12px", fontWeight: 700, textTransform: "uppercase", color: "#888", display: "block", marginBottom: "4px" }}>Priority</label>
                <select value={taskForm.priority} onChange={(e) => setTaskForm({ ...taskForm, priority: e.target.value })} style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box" }}>
              ...
              <div>
                ...
                <input type="date" value={taskForm.dueDate} onChange={(e) => setTaskForm({ ...taskForm, dueDate: e.target.value })} style={{ width: "100%", padding: "8px 12px", border: "1px solid #ddd", borderRadius: "8px", fontSize: "14px", boxSizing: "border-box" }} />
              </div>
            </div>
            <div className="ard-modal-btns" style={{ marginTop: "20px" }}>
              ...
              <button className="btn-verify" onClick={handleTaskSubmit}>Assign New Task</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="detail-header">
        <button 
          className="btn-admin" 
          style={{ padding: "10px 18px", background: "#fff", color: "#1a1a2e", border: "1.5px solid #ddd", display: "flex", alignItems: "center", gap: "8px", whiteSpace: "nowrap" }} 
          onClick={() => navigate("/admin-requests")}
        >
          <span style={{ fontSize: "18px", lineHeight: 1 }}>←</span> Back to Dashboard
        </button>
        <h1 className="detail-title" style={{ margin: 0, fontSize: "32px", lineHeight: "1.2" }}>
          {req.district}{req.fullAddress ? `, ${req.fullAddress}` : ""}
        </h1>
      </div>
      ...
      <div className="detail-layout">
        ...
        <div className="detail-left">
          ...
          <div className="detail-card">
            <h3 style={{ fontSize: "20px", marginBottom: "16px" }}>Request Details</h3>
            <table className="detail-table" style={{ fontSize: "15px" }}>
              <tbody>
                ...
                <tr><td>Priority</td><td><span className={`priority-label priority-${req.priority?.toLowerCase()}`}>{req.priority || "—"}</span></td></tr>
          ...
          {req.additionalDetails && (
            <div className="detail-card" style={{ marginTop: "24px" }}>
              <h3 style={{ fontSize: "20px", marginBottom: "12px" }}>Description</h3>
              <p style={{ fontStyle: "italic", color: "#444", lineHeight: "1.8", fontSize: "15px" }}>"{req.additionalDetails}"</p>
            </div>
          )}
          ...
          <div className="detail-card" style={{ marginTop: "24px" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
              <h3 style={{ margin: 0, fontSize: "20px" }}>
                ...
                {assignedVolunteersFromTasks.length > 0 && (
                  <span style={{ marginLeft: "8px", background: "#c0392b", color: "#fff", borderRadius: "999px", fontSize: "13px", fontWeight: 700, padding: "2px 10px" }}>
              ...
              {canAssignTask && (
                <button
                  ...
                  style={{ background: "#c0392b", color: "#fff", border: "none", borderRadius: "6px", padding: "8px 16px", fontSize: "14px", fontWeight: 600, cursor: "pointer" }}
                >
                  + Assign New Task
                </button>
              )}
            ...
            {loadingTasks ? (
              ...
              <div style={{ textAlign: "center", padding: "32px 0", color: "#aaa" }}>
                ...
                <p style={{ margin: 0, fontSize: "15px" }}>No volunteers assigned yet.</p>
                <p style={{ margin: "4px 0 0", fontSize: "14px" }}>Use "Assign New Task" to dispatch volunteers to this request.</p>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                {assignedVolunteersFromTasks.map((vol, i) => {
                  const volTasks = requestTasks.filter((t) => t.assignedTo?.volunteerEmail === vol.email);
                  return (
                    <div key={i} style={{ background: "#f6fff6", border: "1.5px solid #d1fae5", borderRadius: "10px", padding: "16px 20px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "14px" }}>
                        <div style={{ width: "42px", height: "42px", borderRadius: "50%", background: "#16a34a", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "18px", flexShrink: 0 }}>
                          {vol.name?.[0]?.toUpperCase() || "V"}
                        </div>
                        <div>
                          <p style={{ margin: 0, fontWeight: 700, fontSize: "16px", color: "#1a1a2e" }}>{vol.name}</p>
                          <p style={{ margin: 0, fontSize: "14px", color: "#666" }}>{vol.email}</p>
                        </div>
                        <div style={{ marginLeft: "auto", fontSize: "13px", color: "#16a34a", fontWeight: 600 }}>
                          {volTasks.length} task{volTasks.length !== 1 ? "s" : ""}
                        </div>
                      </div>
                      <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                        {volTasks.map((t) => (
                          <div key={t._id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "#fff", border: "1px solid #e8f4e8", borderRadius: "8px", padding: "10px 14px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              <span style={{ fontSize: "18px" }}>{TASK_ICONS[t.taskType] || "📌"}</span>
                              <div>
                                <p style={{ margin: 0, fontSize: "14px", fontWeight: 600, color: "#1a1a2e" }}>{t.title}</p>
                                <p style={{ margin: 0, fontSize: "12px", color: "#888" }}>{t.taskType}{t.zone ? ` · ${t.zone}` : ""}</p>
                              </div>
                            </div>
                            <div style={{ display: "flex", gap: "8px", alignItems: "center", flexShrink: 0 }}>
                              <span style={{ fontSize: "12px", fontWeight: 700, padding: "4px 8px", borderRadius: "999px", color: "#fff", background: PRIORITY_COLORS[t.priority] || "#888" }}>
                                {t.priority?.toUpperCase()}
                              </span>
                              <span style={{ fontSize: "12px", fontWeight: 600, padding: "4px 10px", borderRadius: "999px", border: `1.5px solid ${TASK_STATUS_COLORS[t.status] || "#888"}`, color: TASK_STATUS_COLORS[t.status] || "#888" }}>
          ...
          {req.assignedVolunteer && (
            <div className="detail-card" style={{ marginTop: "24px" }}>
              <h3 style={{ fontSize: "20px", marginBottom: "16px" }}>Volunteer Messages</h3>
              {(!req.inquiries || req.inquiries.length === 0) ? (
                <p style={{ color: "#aaa", fontSize: "15px", marginBottom: "20px" }}>No messages yet.</p>
              ) : (
                <div style={{ marginBottom: "20px" }}>
                  {req.inquiries.map((msg, i) => (
                    <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: msg.from === "admin" ? "flex-end" : "flex-start", marginBottom: "14px" }}>
                      <div style={{ maxWidth: "80%", padding: "12px 16px", borderRadius: msg.from === "admin" ? "16px 16px 4px 16px" : "16px 16px 16px 4px", background: msg.from === "admin" ? "#c0392b" : "#f0ece4", color: msg.from === "admin" ? "#fff" : "#1a1a1a", fontSize: "15px", lineHeight: "1.6" }}>
                        {msg.message}
                      </div>
                      <span style={{ fontSize: "12px", color: "#aaa", marginTop: "6px" }}>{msg.from === "admin" ? "You (Admin)" : msg.senderName} · {timeAgo(msg.sentAt)}</span>
                    </div>
                  ))}
                </div>
              )}
              <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                <textarea placeholder="Type a reply to the volunteer..." value={replyText} onChange={(e) => setReplyText(e.target.value)} disabled={sendingReply} style={{ width: "100%", border: "1px solid #e8e4dc", borderRadius: "8px", padding: "12px 16px", fontSize: "15px", resize: "vertical", fontFamily: "inherit", boxSizing: "border-box", minHeight: "80px" }} />
                <button className="btn-verify" style={{ alignSelf: "flex-end", opacity: sendingReply || !replyText.trim() ? 0.5 : 1, padding: "10px 20px" }} onClick={handleSendReply} disabled={sendingReply || !replyText.trim()}>
      ...
      <div key={i} style={{ display: "flex", gap: "12px", alignItems: "flex-start", marginBottom: "10px", paddingBottom: "10px", borderBottom: i < req.priorityHistory.length - 1 ? "1px solid #f0f0f0" : "none" }}>
        ...
        <div>
          ...
          {req.status === "volunteer_done" && (
            <div className="detail-card" style={{ marginTop: "24px", background: "#f0fdf4", border: "1px solid #86efac" }}>
              <h3 style={{ margin: "0 0 12px 0", color: "#065f46" }}>Ready to Close</h3>
              <p style={{ fontSize: "15px", color: "#166534", marginBottom: "20px" }}>A volunteer has completed their work.</p>
              <button className="btn-verify" style={{ width: "100%", padding: "14px" }} onClick={handleMarkComplete}>Mark as Completed</button>
            </div>
          )}
...
<div className="detail-card" style={{
  marginTop: "24px",
  background: !aiAnalysis ? "#f8f8f8" :
              aiAnalysis.verdict === "LIKELY_FRAUD" ? "#fef2f2" :
              aiAnalysis.verdict === "SUSPICIOUS"   ? "#fffbeb" : "#f0fdf4",
  border: `1px solid ${
            !aiAnalysis ? "#e0e0e0" :
            aiAnalysis.verdict === "LIKELY_FRAUD" ? "#fca5a5" :
            aiAnalysis.verdict === "SUSPICIOUS"   ? "#fde68a" : "#86efac"}`
}}>
  <h3 style={{ margin: "0 0 8px 0" }}> AI Fraud Analysis</h3>
  {loadingFraud ? (
    <p style={{ fontSize: "14px", color: "#aaa", margin: 0 }}>Analyzing request...</p>
  ) : aiAnalysis ? (
    <>
      <p style={{ fontSize: "28px", fontWeight: 700, margin: "0 0 6px 0" }}>
        {aiAnalysis.fraudScore}<span style={{ fontSize: "14px", color: "#888" }}>/100</span>
      </p>
      <span style={{
        padding: "4px 12px", borderRadius: "999px", fontWeight: 700, fontSize: "13px",
        background: aiAnalysis.verdict === "LIKELY_FRAUD" ? "#c0392b" :
                    aiAnalysis.verdict === "SUSPICIOUS"   ? "#b45309" : "#16a34a",
        color: "#fff"
      }}>
        {aiAnalysis.verdict}
      </span>
      <p style={{ marginTop: "10px", fontSize: "14px", color: "#555", margin: "10px 0 0" }}>
        {aiAnalysis.reason}
      </p>
    </>
  ) : (
    <p style={{ fontSize: "14px", color: "#aaa", margin: 0 }}>Analysis unavailable.</p>
  )}
</div>
          ...
          <div className="detail-card ai-sidebar-card">
            ...
            <button className="ai-sidebar-btn" onClick={() => { setActiveAiView("resource"); setShowAiModal(true); if(!aiResources) handleAiResourceAllocation(); }}>
               Suggest Resource Allocation
            </button>
            <button className="ai-sidebar-btn" onClick={() => { setActiveAiView("volunteer"); setShowAiModal(true); handleAiVolunteerMatch(); }}>
               Find Best Volunteers
            </button>
            <button className="ai-sidebar-btn" onClick={() => { setActiveAiView("donation"); setShowAiModal(true); if(!aiDonation) handleAiDonationAllocation(); }}>
