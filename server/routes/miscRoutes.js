  ...
  try {
    ...
    if (task.assignedTo?.volunteerEmail) {
      try {
        ...
        if (global.io)
          global.io
            .to(task.assignedTo.volunteerEmail)
...
router.post("/api/operations", async (req, res) => {
  try {
    const { supplyPickupPoint, volunteers, maxCapacity } = req.body;

    // ── Capacity Check ────────────────────────────────────────
    // If a collection point is specified, count volunteers already
    // assigned to active operations at that same site and block the
    // request if adding the new group would exceed maxCapacity.
    if (supplyPickupPoint) {
      // Find all non-completed operations at this collection point
      const existingOps = await OperationModel.find({
        supplyPickupPoint: supplyPickupPoint,
        status: { $nin: ["Completed"] },
      });

      // Sum up every volunteer already assigned to that site
      const currentVolunteerCount = existingOps.reduce(
        (sum, op) => sum + (op.volunteers?.length || 0),
        0,
      );

      const incomingCount = volunteers?.length || 0;

      // Determine the cap: prefer the value sent in the request body,
      // but fall back to the cap stored on any existing operation at
      // that site (so the limit is enforced consistently).
      const effectiveCapacity =
        maxCapacity ??
        existingOps.find((op) => op.maxCapacity != null)?.maxCapacity;

      if (
        effectiveCapacity != null &&
        currentVolunteerCount + incomingCount > effectiveCapacity
      ) {
        return res.status(400).json({
          message:
            `Collection point "${supplyPickupPoint}" is over capacity. ` +
            `Currently assigned: ${currentVolunteerCount} volunteer(s). ` +
            `Trying to add: ${incomingCount}. ` +
            `Max allowed: ${effectiveCapacity}.`,
        });
      }
    }
    // ─────────────────────────────────────────────────────────
...
// ── Also enforce capacity when EDITING an existing operation ──────────────
router.put("/api/operations/:id", async (req, res) => {
  try {
    const { supplyPickupPoint, volunteers, maxCapacity } = req.body;

    if (supplyPickupPoint) {
      const existingOps = await OperationModel.find({
        _id: { $ne: req.params.id }, // exclude the operation being edited
        supplyPickupPoint: supplyPickupPoint,
        status: { $nin: ["Completed"] },
      });
      ...
      if (
        ...
        return res.status(400).json({
          message:
            `Collection point "${supplyPickupPoint}" is over capacity. ` +
            `Other active operations at this site have ${currentVolunteerCount} volunteer(s). ` +
            `Trying to assign: ${incomingCount}. ` +
            `Max allowed: ${effectiveCapacity}.`,
        });
      }
    }
...
router.post("/api/ngo/register", async (req, res) => {
  try {
    ...
    if (existing)
      return res
        .status(400)
        .json({ message: "Agency with this email already registered" });
    const hashedPassword = await bcrypt.hash(password, 10);
    const agency = new NGOAgencyModel({
      ...req.body,
      password: hashedPassword,
    });
    await agency.save();
    res.status(201).json({
      message: "Agency registered. Awaiting admin verification.",
      agency,
    });
  } catch (err) {
    res.status(500).json({ message: "Registration failed" });
...
router.post("/api/ngo/login", async (req, res) => {
  try {
    ...
    if (agency.status !== "verified")
      return res
        .status(403)
        .json({ message: "Your agency is not yet verified by admin." });
    const isMatch = await bcrypt.compare(password, agency.password);
    if (!isMatch)
      return res.status(401).json({ message: "Incorrect password" });
    res.status(200).json({
      message: "Login successful",
      agency: { ...agency._doc, password: undefined },
    });
  } catch (err) {
    res.status(500).json({ message: "Login failed" });
  }
...
router.get("/api/stats", async (req, res) => {
  try {
    ...
    const availableFunds =
      (collected[0]?.total || 0) - (utilized[0]?.total || 0);
